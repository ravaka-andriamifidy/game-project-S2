...
/*
* This class is to improve the movement of the chaser
...
class Chaser(_initialPosition: Vector2) extends Entity(_initialPosition) with DrawableObject with Movable{
  ...
  def this(x: Int, y: Int) = {
    ...
    ss = new Spritesheet("SpriteSheet/PythonSpriteSheetNoBg.png", Constants.SPRITE_WIDTH, Constants.SPRITE_HEIGHT)
  }
  ...
  def go(direction: Direction): Unit = {
    move = true
    direction match {
      case Direction.RIGHT => this.getNewPosition.add(Constants.SPRITE_WIDTH, 0)
      case Direction.LEFT => this.getNewPosition.add(-Constants.SPRITE_WIDTH, 0)
      case Direction.UP => this.getNewPosition.add(0, Constants.SPRITE_HEIGHT)
      case Direction.DOWN => this.getNewPosition.add(0, -Constants.SPRITE_HEIGHT)
      case _ =>
    }
    turn(direction)
  }
